...
public class Board_leacter {

	public static void main(String[] args) {
		new Board_leacter().start();
	}

	ArrayList<HashMap<String, Object>> boardTable = new ArrayList<HashMap<String, Object>>(); //이중 ArrayList 생성
	SimpleDateFormat format = new SimpleDateFormat("MM-dd"); // 날짜 형식을 바꾸기 위한 메서드
	...
	private void insert() {
		...
		board.put("BOARD_NO", max + 1); // board 테이블을 찾아서 가장 큰 값에 +1을 하는 방법
	...
	private void read() {
		...
		switch (input) {
		case 1:
			update(board); // 수정이나 삭제 시 사용자가 선택한 게시글을 파라미터로 선택
			break;
		case 2:
			delete(board);
			break;
